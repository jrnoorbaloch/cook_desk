...
const BUNDLED_AGENT_VERSION = "1.1.0";

function _versionLessThan(left, right) {
    const parse = value => String(value).split(".").map(part => parseInt(part, 10) || 0);
    const a = parse(left);
    const b = parse(right);
    return [0, 1, 2].some((index) => a[index] < b[index] && a.slice(0, index).every((part, i) => part === b[i]));
}
    ...
    frappe.call({
        ...
        callback(r) {
            if (r.message && r.message.running) {
                ...
                if (r.message.version && _versionLessThan(r.message.version, BUNDLED_AGENT_VERSION)) {
                    frappe.show_alert({
                        message: __("Update available: agent {0}, bundled version {1}", [r.message.version, BUNDLED_AGENT_VERSION]),
                        indicator: "orange",
                    });
                }
                if (show_message) {
