...
async function listPrinters() {
    if (process.platform === "win32") {
        ...
        try {
            ...
            const printers = (Array.isArray(rows) ? rows : [rows]).map(row => ({
                name: String(row.Name || "").trim(),
                is_default: Boolean(row.Default),
                status: "Ready",
            })).filter(row => row.name);
            if (printers.length > 0) return printers;
        } catch (e) {
            console.error("PowerShell printer detection error:", e.message);
        }
    }
...
function getDefaultPrinterName() {
    if (process.platform === "win32") {
        ...
        try {
            const out = execSync(
                "powershell -NoProfile -Command \"(Get-Printer | Where-Object Default).Name\"",
                { encoding: "utf-8", timeout: 5000 }
            ).trim();
...
// ─── Test Print ───────────────────────────────────────────────────────────────
// 80mm thermal printer = 42 chars per line (at 12cpi / font size 9-10)
const WIDTH = 34;
...
async function testPrint() {
    ...
    const lines = [
        "",
        _line("="),
        _center("USB PRINT AGENT"),
        _center("COOK DESK — TEST RECEIPT"),
        _line("="),
        _row("Date:", now.toLocaleDateString()),
        _row("Time:", now.toLocaleTimeString()),
        _row("Port:", String(config.port)),
        _row("Host:", os.hostname()),
        _row("Printer:", (printerName || "default").substring(0, 20)),
        _line("-"),
        _center("*** AGENT WORKING ***"),
        _line("-"),
        _center("*** Developed By: Pixymo Tech ***"),
        _line("="),
        "", "", "",
    ].join("\n");
