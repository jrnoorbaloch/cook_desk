...
async function pairWithErpNext() {
    const site = erpNextUrlInput.value.trim().replace(/\/$/, "");
    if (!site || !/^https?:\/\//i.test(site)) {
        log("Enter a valid ERPNext URL", "error");
        return;
    }
    const granted = await chrome.permissions.request({ origins: [`${site}/*`] });
    if (!granted) {
        log("ERPNext permission was not granted", "error");
        return;
    }
    try {
        const response = await fetch(`${site}/api/method/cook_desk.usb_printing.api.usb_agent.get_agent_credentials`, { credentials: "include" });
        const result = await response.json();
        const credentials = result.message || result;
        if (!credentials.token) throw new Error("ERPNext did not return an agent token");
        await chrome.storage.local.set({ agentIp: credentials.ip, agentPort: credentials.port, agentToken: credentials.token, erpNextUrl: site });
        agentIpInput.value = credentials.ip;
        agentPortInput.value = credentials.port;
        log("Paired with ERPNext successfully", "success");
        checkStatus(true);
    } catch (error) {
...
async function loadConfig() {
    ...
    erpNextUrlInput.value = data.erpNextUrl;
}
...
btnPair.addEventListener("click", pairWithErpNext);
